...
public class HelloServlet implements Servlet {
  ...
  // 1) Servlet Container가 서블릿 객체를 만든 후 호출하는 메서드
  // => 각 서블릿 클래스에 대해 인스턴스는 딱 한 개만 만든다.
  // => 따라서 이 메서드는 각 서블릿에 대해 딱 한 번만 호출된다.
  // => 주로 서블릿 객체가 작업할 때 사용할 자원을 준비하는 코드를 넣는다.
  @Override
  public void init(ServletConfig config) throws ServletException {
    // 파라미터로 받은 ServletConfig 객체는 잘 보관해 두었다가
    // getServletConfig() 메서드가 호출될 때 이 객체를 리턴하라!
    this.config = config;
    
    // 혹시 이 서블릿을 실행하는데 필요한 객체나 데이터가 있다면 여기에서 준비하라!
    
    // 일단 메서드 호출된 것을 확인하기 위해 서버 콘솔 창으로 간단한 내용을 출력한다.
    System.out.println("init()");
  }
  
  
  
  // 2) 클라이언트 요청이 들어 올 때마다 Servlet Container가 호출하는 메서드 
  // => 이 메서드가 가장 중요하다.
  // => 클라이언트 요청이 있을 때 마다 호출되는 메서드이기 때문에
  //    이 메서드 안에 요청을 처리하는 코드를 둔다.
  @Override
  public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
    // 서블릿 컨테이너는 이 메서드를 호출하면서 작업할 때 사용하라고, 
    // 아주 중요한 두 개의 객체를 파라미터로 넘겨준다.
    // ServletRequest : 요청 정보를 다룰 때 사용할 도구들이 들어 있다.
  ...
  // => 서블릿 컨테이너가 종료되기 직전에 딱 한 번 호출된다.
  // => 보통 init()에서 준비한 자원을 해제시키는 작업을 수행한다.
  @Override
  public void destroy() {
    // 특별히 서버를 멈추기 전에 해제해야 할 자원이 있다면, 
    // 이 메서드에 그 코드를 작성하라!
